# reciprocal pulse counter from 'horuable'
...
if __name__ == "__main__":
    ...
    # freq(MFREQ)      # set CPU frequency; not necessarily the default 125 MHz    - freq does nto take agrs in this version
    update_flag = False
    ...
    def counter_handler(sm):
        ...
        global update_flag
        if not update_flag:
            gateVal = int(MFREQ/1) # orignally 1000 decrase value to slow down
            sm0.put(gateVal)
            sm0.exec("pull()")
    ...
    while True:
        if update_flag:
            ...
            if True:
              ...
              freq = pulse_count * ( MCAL / clock_count)  # calibration constant
              # print("{0:d},{1:d},{2:d},{3:0.2f} Hz".format(i,clock_count,pulse_count,freq))
              uart.write("{0:d},{1:f}\r\n".format(i,freq))
              i += 1
